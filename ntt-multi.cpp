...
const unsigned int N = 300100, P = 998244353, G = 3;
...
template <typename T> void ntt(T *y, int len, int sign)
{
    for (int stride = 2; stride <= len; stride <<= 1)
    {
        int gn = mod_pow(G, (P - 1) / stride);
        if (sign == -1)
            gn = mod_pow(gn, P - 2);
        ...
        for (int step = 0; step < len; step += stride)
        {
            ...
            {
                int u = y[k] % P;
                int v = 1ll * g * y[k + stride / 2] % P;
                y[k] = (u + v) % P;
                y[k + stride / 2] = (u - v + P) % P;
                g = (1ll * g * gn) % P;
            }
        }
    }
    ...
    if (sign == -1)
    {
        ...
        for_each(y, y + len, [inv](T &y_ele) { y_ele = 1ll * y_ele * inv % P; });
    }
}
...
vector<int> fft_multi(vector<int> &f, vector<int> &g)
{
    ...
    vector<int> y_f(2 * max_len);
    vector<int> y_g(2 * max_len);
    transform(f.begin(), f.end(), y_f.begin(), [](int x) -> int { return x; });
    transform(g.begin(), g.end(), y_g.begin(), [](int x) -> int { return x; });

    print_c(y_f);
    print_c(y_g);

    cout << "ntt transform, butterfly_exchanging ......, y_f.size(): " << y_f.size() << endl;
    butterfly_exchange(y_f.data(), y_f.size());
    butterfly_exchange(y_g.data(), y_g.size());
    print_c(y_f);
    print_c(y_g);

    cout << "ntt transform, sampling ......" << endl;
    ntt(y_f.data(), y_f.size(), 1);
    ntt(y_g.data(), y_g.size(), 1);
    print_c(y_f);
    print_c(y_g);
